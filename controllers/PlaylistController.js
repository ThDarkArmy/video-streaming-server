...
export const getAllPlaylistByChannel = async (req, res, next) => {
  try {
    const playlists = await Playlist.find({ channel: req.params.id }).populate(
      "videos",
      "-__v"
    );
    res.status(200).json({
...
export const addVideoToPlaylist = async (req, res, next) => {
  try {
    ...
    if (playlist.videos.includes(videoId))
      throw createError.BadRequest("Video already exists in the playlist.");
...
export const removeVideoFromPlaylist = async (req, res, next) => {
  try {
    ...
    if (!playlist.videos.includes(videoId))
      throw createError.NotFound("Video doesn't exists in the playlist.");
